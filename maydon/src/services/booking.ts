...
import type { Booking, BookingStatus } from "./models.ts";
import {
  kv,
  keys,
  getBookingsByDay,
...
export async function validateBookingRequest(
  ...
  if (duration < settings.minDurMin) {
    return { valid: false, error: `Minimum duration is ${settings.minDurMin} minutes` };
  }
  if (duration > settings.maxDurMin) {
    return { valid: false, error: `Maximum duration is ${settings.maxDurMin} minutes` };
  }
  ...
  if (startMin < openMin || endMin > closeMin) {
    return { valid: false, error: `Outside working hours (${settings.openTime}-${settings.closeTime})` };
  }
  ...
  const daysDiff = Math.floor((bookingDate.getTime() - localToday.getTime()) / (1000 * 60 * 60 * 24));
  
  if (daysDiff < 0 || daysDiff >= settings.horizonDays) {
    return { valid: false, error: `Booking horizon is ${settings.horizonDays} days` };
  }
  ...
  if (duplicate) {
    return { valid: false, error: "You already have a pending request for this slot" };
  }
...
export async function confirmBooking(id: string): Promise<{ success: boolean; error?: string }> {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    if (checkOverlapWithConfirmed(dayBookings, b.start, b.end)) {
    ...
    const ok = await kv.atomic()
      .check(dayVer)
      .set(keys.booking(id), { ...b, status: "confirmed", decidedAt: new Date().toISOString() })
